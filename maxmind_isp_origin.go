...
func MaxMindHeader(valueDescription, valueType string, dbMetadata maxminddb.Metadata) (string, error) {
	// TODO can this be a global const or something...
	tmpl, err := template.New("tmpl").Parse(`#
# MaxMind IP to {{.ValueDescription}} mapping
# 
...
func (m *MaxMindIspOrigin) headers() error {
	asn, err := MaxMindHeader("Autonomous System Number (ASN)", "ASN", m.db.Metadata)
	if err != nil {
		return err
	}

	if err = m.asn.Header(asn); err != nil {
		return err
	}

	isp, err := MaxMindHeader("Internet Service Provider (ISP)", "ISP", m.db.Metadata)
	if err != nil {
		return err
	}

	return m.isp.Header(isp)
}
